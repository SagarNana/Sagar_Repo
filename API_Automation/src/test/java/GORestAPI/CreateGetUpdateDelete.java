...
// Create the object of pojo class for create user 
		...
		CreateUserResponse createUserResponse = given().log().all().header("Content-Type", "application/json")
				.header("Authorization", "Bearer 0431b2f96b1525e9b03c6dff1a4b2474b662d2dba722b3328112654c5d6fbdda")
				.body(createUser).when().post("public/v2/users").then().log().all().extract()
...
//		https://gorest.co.in/public/v2/users/userId  (userid mtlun humne jo create ki post method se vho peste krni hai )
		...
		GetUserResponse getUserResponse = given().log().all().pathParam("userId", createUserId) // ya per hume userId
																								...
																								// provide kr rhe hai
				.header("Authorization", "Bearer 0431b2f96b1525e9b03c6dff1a4b2474b662d2dba722b3328112654c5d6fbdda")
				.when().get("public/v2/users/{userId}").then().log().all().extract().as(GetUserResponse.class);
...
// https://gorest.co.in/public/v2/users/userId
		...
		UpdateResponse updateResponse = given().log().all().pathParam("userId", createUserId)
				...
				.header("Authorization", "Bearer 0431b2f96b1525e9b03c6dff1a4b2474b662d2dba722b3328112654c5d6fbdda")
				.body(updateUser).when().put("public/v2/users/{userId}").then().log().all().extract()
...
// https://gorest.co.in/public/v2/users/userId
		...
		given().log().all().pathParam("userId", createUserId)
				.header("Authorization", "Bearer 0431b2f96b1525e9b03c6dff1a4b2474b662d2dba722b3328112654c5d6fbdda")
				.when().delete("public/v2/users/{userId}").then().log().all().extract();
